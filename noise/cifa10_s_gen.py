...
class ImageTransfer:
    def __init__(self, root_dir=None, meta_file=None, save_root=None, decoder_type='pil',
                 resize_type='pil-bilinear', resize=32, transform_type='val', return_online=False, file_path=None):
    ...
    def image_resize(self, img, y, x, h, w):
        ...
        if self.transform_type == 'train':
            ...
            if 'pil' in self.resize_type:
                frist_resize = tuple(int(size * 8 / 7) for size in self.resize)
    ...
    def read_file(self, meta_dict):
        filebytes = np.fromfile(meta_dict['filename'], dtype=np.uint8)  #读取图片路径
        return filebytes
...
if __name__ == '__main__':
    ...
    # image_gen = ImageTransfer(root_dir='/mnt/sunjialiang/AAAD/noise/original', meta_file='val.txt',
    #                           save_root='', decoder_type='opencv',
    #                           transform_type='val', resize_type='pil-hamming')
    # numpy_image, label = image_gen.getimage(0)
    # generate numpy image on index 0
    images = []
    # for i in range(10000):
    imgdir = '/mnt/jfs/sunjialiang/AAAD/noise_visualization/CIFAR10/original_cifar10/train_cifar10/'
    for imgfile in os.listdir(imgdir):
        path = os.path.join('/mnt/sunjialiang/AAAD/noise/original_cifar10/train_cifar10/', imgfile)
        imagS = ImageTransfer(file_path=path, decoder_type='opencv', resize_type='pil-hamming',
                          transform_type='val', return_online=True)
        noise = imagS.getimage()
        noise = torch.from_numpy(noise)
        noise = noise.permute(2, 0, 1)
        noise = noise.unsqueeze(0)
        noise = noise.numpy()
        adv_images = noise.astype(np.uint8)
        images.extend(adv_images)
        print(noise)
    # images = np.array(images)
    # images = np.transpose(images, (0, 2, 3, 1))
    # # 0 32 32 3
    # np.save('cifar_system.npy', images)
    #
